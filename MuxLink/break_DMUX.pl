...
while (<FH>) {
    ...
    push @{ $connect_from_correct{ $columns[0] } },
      $columns[0] . " " . $columns[1];
    push @{ $connect_from_correct_val{ $columns[0] } }, $columns[2];
    push @{ $connect_to_correct{ $columns[1] } },
      $columns[0] . " " . $columns[1];
}
close(FH);
    ...
    if ( $length == 1 ) {
        foreach my $key_two ( keys %to_check_key_bits ) {
            ...
            if ( ( $path0_two eq $path0 ) && ( $path1_two eq $path1 ) ) {
                ...
                my $lik0_two = $connect_from_all{ $cells{$path0} . " "
                      . $cells{$output_two} };
                my $lik1_two = $connect_from_all{ $cells{$path1} . " "
                      . $cells{$output_two} };
                ...
                if ( $dif >= $th || $dif2 >= $th ) {
                    ...
                    }    #end if difference 1 is greater than dif2
                    elsif ( $dif2 > $dif ) {
                    ...
                    }    #end if difference 2 is greater than dif2
                    else {    #they are equal
                        if ( $lik0 > $lik1 ) {
                            if ( $lik0 > $lik0_two ) {
                                $dec_key_bits{$key}     = "0";
        ...
        if ( $flag == 0 ) {
            if ( $dif >= $th ) {
                if   ( $lik0 > $lik1 ) { $dec_key_bits{$key} = "0"; }
        ...
        my $output_two  = $columns_two[0];
        my $lik0_two =
          $connect_from_all{ $cells{$path0} . " " . $cells{$output_two} };
        my $lik1_two =
